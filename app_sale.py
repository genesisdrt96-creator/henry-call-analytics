...
# Tiêu đề báo cáo của bạn
st.title("🚀 Thống kê Data Ringcentral - Dream Talent")
st.markdown("---")

# --- 1. DANH SÁCH MÃ VÙNG ĐẦY ĐỦ 50 TIỂU BANG ---
AC_TO_STATE = {
...
def categorize_depth(sec):
    ...
    if sec < 30: return "1. Short (< 30s)"
    if sec < 300: return "2. Potential (30s-5m)"
    if sec < 900: return "3. Engaged (5m-15m)"
    return "4. Closing (> 15m)"

# --- 3. TẢI VÀ KIỂM TRA FILE ---
...
if uploaded_file is not None:
    ...
    m3.metric("🇺🇸 Bang chủ lực", state_counts['Bang'].iloc[0] if not state_counts.empty else "N/A")
    ...
    # --- 6. CHIỀU SÂU CUỘC GỌI ---
    st.subheader("📊 Phân tích Độ sâu Cuộc hội thoại (Conversation Depth)")
    ...
    with cd1:
        fig_d = px.bar(depth_sum, x='Phân lớp', y='Số lượng', color='Phân lớp', text_auto=True)
        st.plotly_chart(fig_d, use_container_width=True)
    with cd2:
        fig_p = px.pie(depth_sum, values='Số lượng', names='Phân lớp', hole=0.4)
        st.plotly_chart(fig_p, use_container_width=True)

    st.divider()

    # --- 7. GIỜ VÀNG (LOGIC CHUẨN) ---
    st.subheader("⏰ Tỷ lệ khách sẵn sàng nói chuyện lâu (>15p) theo giờ")
    h_conn = df_conn.groupby('Hour').size()
    h_gold = df_conn[df_conn['Sec'] >= 900].groupby('Hour').size()
    h_stats = pd.DataFrame({'Nhấc máy': h_conn, 'Vàng': h_gold}).fillna(0)
    h_stats['Tỷ lệ %'] = 0.0
    mask = h_stats['Nhấc máy'] > 0
    h_stats.loc[mask, 'Tỷ lệ %'] = round((h_stats.loc[mask, 'Vàng'] / h_stats.loc[mask, 'Nhấc máy']) * 100, 1)
    
    if not h_stats.empty:
        reliable = h_stats[h_stats['Nhấc máy'] >= 5]
        if not reliable.empty:
            best = reliable.loc[reliable['Tỷ lệ %'].idxmax()]
            st.info(f"💡 **Giờ Vàng:** Khung giờ **{int(best.name)}h** có tỷ lệ chốt đơn tiềm năng cao nhất (**{best['Tỷ lệ %']}%** khách nhấc máy nói trên 15p).")

    fig_t = px.line(h_stats.reset_index(), x='Hour', y='Tỷ lệ %', markers=True)
    fig_t.update_layout(yaxis_range=[0, 100])
    st.plotly_chart(fig_t, use_container_width=True)

    st.divider()

    # --- 8. BẢNG CHI TIẾT & TẢI VỀ ---
    st.subheader("📋 Bảng hiệu suất chi tiết nhân viên")
    report = df_out.groupby('Staff_Name').agg(
        Tong_goi=('Direction', 'count'),
        Short=('Sec', lambda x: ((x > 0) & (x < 30)).sum()),
        Potential=('Sec', lambda x: ((x >= 30) & (x < 300)).sum()),
        Engaged=('Sec', lambda x: ((x >= 300) & (x < 900)).sum()),
        Closing=('Sec', lambda x: (x >= 900).sum())
    ).reset_index()
    report = report.sort_values('Closing', ascending=False)
    report.columns = ['Nhân Viên', 'Tổng gọi', 'Short (<30s)', 'Potential (30s-5m)', 'Engaged (5-15p)', 'Closing (>15p)']
    ...
    st.info("👋 Chào Henry Team! Vui lòng tải file CSV Call Log để hệ thống bắt đầu phân tích.")
